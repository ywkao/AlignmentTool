...
void ESAlignTool::initAllPara(const edm::ParameterSet& iConfig)
{
    ...
    _evt_TooManyReHits=0;
    _evt_TooManySelTracks_fill_tracks=0;
    _evt_TooManySelTracks_fill_PredictionState=0;
...
void ESAlignTool::fill_esRecHit(const CaloGeometry *caloGeom, edm::Handle<ESRecHitCollection> PreshowerRecHits)
{
    ...
    for ( esrh_it = PreshowerRecHits->begin(); esrh_it != PreshowerRecHits->end(); esrh_it++)
    {
        ...
        if(Nesrh>=10000)
        {
            if( _debug ) edm::LogWarning("fill_esRecHit")<<"Too many ES RecHits.\n";
            _evt_TooManyReHits++;
            continue;
        }
...
void ESAlignTool::fill_tracks(edm::Handle<reco::TrackCollection> TrackCol)
{
    ...
    {    
        if(Ntrack>=2000)
        {
            if( _debug )edm::LogWarning("fill_tracks")<<"Too many selected tracks.\n";
            _evt_TooManySelTracks_fill_tracks++;
            continue;
        }
        if ( itTrack->charge()!=0 )
        {
...
void ESAlignTool::fill_PredictionState(int iz, int ip, int idee, edm::Handle<reco::TrackCollection> TrackCol, edm::ESHandle<MagneticField> theMagField, edm::ESHandle<GlobalTrackingGeometry> theTrackingGeometry, edm::ESHandle<Propagator> shPropAlong )
{
    ...
    if( (iz==1||iz==-1) && (ip==1||ip==2) )
    {
        ...
        {    
            ...
            {
                if( _debug ) edm::LogWarning("fill_tracks")<<"Too many selected tracks.\n";
                _evt_TooManySelTracks_fill_PredictionState++;
                continue;
            }
            if ( itTrack->charge()!=0 )
            {
...
ESAlignTool::endJob() {
    ...
    std::cout << " Events too many 10000 reHits " <<_evt_TooManyReHits<<std::endl; 
    std::cout << " Events too many sel track in fill_tracks " <<_evt_TooManySelTracks_fill_tracks<<std::endl; 
    std::cout << " Events too many sel track in fill_PredictionState " <<_evt_TooManySelTracks_fill_PredictionState<<std::endl; 
    std::cout << " --------------------------------------------- " << std::endl;
